...
class MainWindow(QMainWindow):
    ...
    def _on_file_selected(self, path: str):
        ...
        self._file_hint.setText(f"{icon}  {p.name}  ·  {size}  ·  Reading file…")
        self._log.append_log(f"FILE: {p.name} ({size}) loaded")
        readable_extensions = {
            ".pdf", ".jpg", ".jpeg", ".png", ".webp", ".bmp", ".gif", ".tif", ".tiff",
        }
        if p.suffix.lower() in readable_extensions and self.on_file_uploaded:
            threading.Thread(
                target=self.on_file_uploaded,
                args=(path,),
                daemon=True,
            ).start()
            return
