...
from core.chat_memory          import append_to_history, get_recent_history_context
...
_ORDINAL_WORDS = {
    "first": 1, "১ম": 1, "প্রথম": 1,
    "second": 2, "২য়": 2, "দ্বিতীয়": 2,
    "third": 3, "৩য়": 3, "তৃতীয়": 3,
    "fourth": 4, "৪র্থ": 4, "চতুর্থ": 4,
    "fifth": 5, "৫ম": 5, "পঞ্চম": 5,
}


def _document_command_hint(text: str) -> str:
    """Make common Bengali/Banglish page/question requests unambiguous."""
    original = str(text or "").strip()
    lowered = original.lower().translate(_BENGALI_DIGITS)
    if not original:
        return original

    def _number_before(words: str):
        match = re.search(
            rf"(?<!\w)(\d+)\s*(?:st|nd|rd|th)?\s*"
            rf"(?:(?:no\.?|number|নং|নম্বর)\s*)?(?:{words})(?!\w)",
            lowered,
            flags=re.IGNORECASE,
        )
        return match.group(1) if match else None

    page_number = _number_before(r"page|pages|pg|পৃষ্ঠা|পেজ")
    if not page_number:
        for word, number in _ORDINAL_WORDS.items():
            if re.search(
                rf"(?<!\w){re.escape(word)}\s*(?:page|পৃষ্ঠা|পেজ)",
                lowered,
                flags=re.IGNORECASE,
            ):
    ...
    question_number = _number_before(
        r"no\.?|number|নং|নম্বর|নম্বরের"
    )
    if not question_number:
        match = re.search(
            r"(?<!\w)(?:question|ques|প্রশ্ন)\s*"
            r"(?:no\.?|number|নং|নম্বর)?\s*(\d+)(?!\w)",
            lowered,
            flags=re.IGNORECASE,
        )
        question_number = match.group(1) if match else None

    if not page_number and not question_number:
        return original

    wants_pdf = bool(re.search(
        r"\bpdf\b|desktop|save|বানায়|বানিয়ে|বনায়|সেভ|ডেস্কটপ",
        lowered,
        flags=re.IGNORECASE,
    ))
    lines = [
        "\n\n[STRUCTURED DOCUMENT INTENT — follow this instead of asking the user to repeat]\n",
        "The active uploaded document is the source; do not use browser, web search, or screen capture.\n",
    ]
    if page_number:
        lines.append(
            f"Requested page: {page_number}. Pass page_start={page_number} and "
            f"page_end={page_number}.\n"
        )
    if question_number:
        lines.append(
            f"Requested question number: {question_number}. Pass "
            f"question_number={question_number}.\n"
        )
    lines.append(
        "First call document_engine with mode='question' and the active file path.\n"
    )
    if wants_pdf:
        lines.append(
            "The user wants the exact extracted question text saved as a Desktop PDF. "
            "After retrieval, call file_processor with action='create_pdf' and put "
            "the retrieved question text unchanged in content.\n"
        )
    lines.append(
        "Never ask the user to read or retype the question when the file is uploaded."
    )
...
class JarvisLive:
    def __init__(self, ui: JarvisUI):
        ...
        self.ui.on_file_uploaded = self.handle_uploaded_file
    ...
    def handle_uploaded_file(self, file_path: str):
        """Register an uploaded document without blocking the live conversation.

        Images are sent once to Gemini Vision because diagrams need visual
        understanding. PDFs are deliberately NOT pushed page-by-page into the
        Live session: large uploads can exhaust the Live context and make the
        microphone and typed-command loop appear frozen. PDF reading happens
        locally through document_engine when the user asks a question.
        """
        import asyncio
        import base64
        import io
        ...
        if not path or not path.is_file():
            self.ui.write_log("FILE: The selected file could not be found.")
            return
        if not loop or not loop.is_running() or not session:
            self.ui.write_log("FILE: The assistant is not connected yet; upload the file again after it connects.")
            return

        def _send_parts(parts, *, turn_complete):
            ...
            # Upload callbacks run outside the Qt thread. Never wait forever
            # here: a slow Live request must not make the file-upload worker
            # look like the whole assistant stopped.
        ...
        try:
            if ext in image_extensions:
                self._active_document_path = str(path)
                ...
                with Image.open(path) as source:
                    ...
                    # Keep small print legible, while bounding very large uploads.
                    image.thumbnail((4096, 4096), Image.Resampling.LANCZOS)
                    encoded = io.BytesIO()
                    image.save(encoded, format="JPEG", quality=90, optimize=True)

                prompt = (
                    f"The user uploaded the image '{path.name}'. Read all visible "
                    "text carefully, including small print, labels, tables, and "
                    "equations. Understand the image, then briefly tell the user "
                    "in natural Bengali what it contains and that they can ask "
                    "about any part. If something is unreadable, say so clearly. "
                    "Do not use web search or refer to the computer screen."
                )
                _send_parts(
                    [
                        {
                ...
                self.ui.write_log(f"FILE: Image text read — {path.name}")
                return

            if ext != ".pdf":
                self.ui.write_log(
                    "FILE: Text reading supports PDF, JPG, PNG, WEBP, BMP, GIF, and TIFF."
                )
                return
            ...
            with fitz.open(str(path)) as document:
                ...
                if page_count == 0:
                    self.ui.write_log("FILE: This PDF has no pages to read.")
                    return
                selectable_pages = sum(
                    bool((page.get_text("text") or "").strip())
                    for page in document
                )

            # The path is intentionally included: subsequent tool calls can
            # use the local file directly, without sending the entire PDF back
            # through Gemini Live on every turn.
            ready_message = (
                "[DOCUMENT_READY]\n"
                f"Name: {path.name}\n"
                f"Full path: {path}\n"
                f"Pages: {page_count}\n"
                f"Selectable-text pages: {selectable_pages}\n"
                "The PDF is available locally through the document_engine tool. "
                "Do not claim that every page is already in Live context. "
                "When the user asks a page/question/year question, call "
                "document_engine with the saved full path. Use mode='question' "
                "for a page/question number, mode='search' for a topic, "
                "mode='repeated' for duplicates, and mode='export_questions' "
                "or mode='export_repeated' when a Desktop PDF is requested. "
                "If a page has no selectable text, keep ocr=true. "
                "Tell the user briefly, in the user's language, that the PDF "
                "is ready and ask what they want to find."
            )
            _send_parts([{"text": ready_message}], turn_complete=True)
            self.ui.write_log(
                f"FILE: PDF registered locally — {path.name} ({page_count} page(s)); "
                "Live context was not flooded"
            )
        except Exception as e:
            self.ui.write_log(f"FILE: Could not read {path.name}: {type(e).__name__}: {e}")
            print(f"[Upload reading error] {type(e).__name__}: {e}")
    ...
    def _on_text_command(self, text: str):
        ...
        asyncio.run_coroutine_threadsafe(
            self.session.send_client_content(
                turns={"role": "user", "parts": [{"text": _document_command_hint(text)}]},
                turn_complete=True
            ),
            self._loop
        )
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        _addr = (
            f"PRIMARY USER PROFILE NAME: {_user_name}. This profile name is not "
            "visual identity proof. Never infer that a person in a webcam frame, "
            "near the computer, or speaking nearby is this person. Use this name "
            "only after the person explicitly identifies themselves or the app "
            "reports a verified face match; otherwise use neutral language."
            if _user_name
            else
            "PRIMARY USER IDENTITY: unknown. Do not invent or guess a name from "
            "a camera frame, voice, or proximity. Use neutral language unless the "
            "speaker explicitly identifies themselves."
        )
        identity_ctx = (
            f"[IDENTITY]\n"
        ...
        recent_ctx = get_recent_history_context()
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "recall_memory":
                ...
                # শুধুমাত্র ডকুমেন্ট টুলগুলোর জন্য ফাইল পাথ চেক করবে, ইউটিউব বা অন্য টুলের জন্য নয়
                if name in {"file_processor", "document_engine"} and not args.get("file_path"):
                    active_file = (
                        getattr(self.ui, "current_file", None)
                        or self._active_document_path
                    )
                    if active_file:
                        args["file_path"] = active_file
                
                # ইউটিউব বা অন্যান্য টুলের ক্ষেত্রে যেন ভুল করে ফাইল পাথ পাস না হয়
                if name == "youtube_video" and "file_path" in args:
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
           ...
           # ── Barge-in (ভয়েস ইন্টারাপ্ট) ────────────────────────────────────
            if jarvis_speaking:
                try:
                    level = _pcm_level(indata)
                    # ইউজার কথা বললে বা আওয়াজ করলেই সাথে সাথে ইন্টারাপ্ট করবে
                    if level > 0.60:
                        print(f"\n[JARVIS] 🛑 মুখের আওয়াজ শনাক্ত হয়েছে (Level: {level:.2f}) — থামাচ্ছি!")
        ...
        try:
            while True:
                async for response in self.session.receive():
                    ...
                    if response.data:
                        print(f"[JARVIS] 🔊 Audio received: {len(response.data)} bytes")
...
# ── DIRECT SHUTDOWN ──
                                _cmd = txt.lower()
                                shutdown_words = ["ল্যাপটপ বন্ধ", "পিসি বন্ধ", "কম্পিউটার বন্ধ", "শাটডাউন", "shutdown", "power off", "turn off"]
                                if any(w in _cmd for w in shutdown_words):
                                    import subprocess
                                    subprocess.Popen(["shutdown", "/s", "/f", "/t", "3"], shell=True)

                        if sc.turn_complete:
                            print("[JARVIS] ✅ Turn complete")
                            if self._turn_done_event:
                            ...
                            if full_in:
                                ...
                                try:
                                    append_to_history("user", full_in)
                                except Exception:
                                    pass
                                if self._dashboard:
                            ...
                            if full_out:
                                ...
                                try:
                                    append_to_history("model", full_out)
                                except Exception:
                                    pass
                                if self._dashboard:
                                    asyncio.create_task(self._dashboard.broadcast({
   ...
   # ── Startup Face Verification & Fast Greeting ─────────────────────────────

    async def _startup_face_check(self) -> None:
        ...
        try: cam_bytes = await asyncio.wait_for(asyncio.to_thread(_snap_fast), timeout=1.5)
        except asyncio.TimeoutError: cam_bytes = None
        # Unknown is the safe default. A missing camera, a recognition error,
        # or an unexpected classifier result is never proof that the person is
        # Pavel/Boss.
        user_status = 'unknown'
        if cam_bytes:
            try:
                from core.face_id import check_if_pavel
                res = await asyncio.wait_for(asyncio.to_thread(check_if_pavel, cam_bytes), timeout=2.0)
                user_status = 'boss' if res == 'pavel' else ('stranger' if res == 'stranger' else 'nobody')
            except Exception: user_status = 'unknown'
        else: user_status = 'unknown'
        import base64 as _b64
        parts = []
        if cam_bytes:
            parts.append({'inline_data': {'mime_type': 'image/jpeg', 'data': _b64.b64encode(cam_bytes).decode('ascii')}})
        cur_hour = datetime.now().hour
        time_greeting = 'শুভ সকাল' if 5<=cur_hour<12 else ('শুভ দুপুর' if 12<=cur_hour<16 else ('শুভ বিকেল' if 16<=cur_hour<19 else ('শুভ রাত্রি' if 19<=cur_hour<23 else 'অনেক রাত হয়েছে')))
        if user_status == 'boss':
            instruction = f'Look closely at Boss in the attached webcam picture. Notice his face expression, posture, mood, tiredness, or smile. Greet him as your beloved Boss ({time_greeting}) and react naturally and sweetly to his exact expression or how he looks right now (e.g. if he looks tired, show deep care; if smiling, be happy). Keep it strictly to 1 or 2 natural, heartfelt Bengali sentences. Never ask robotic questions like "কোনো সাহায্য লাগবে?".'
        elif user_status == 'stranger':
            instruction = 'An unknown person is seen in the camera. Politely and sweetly ask who they are in 1 short Bengali sentence.'
        else:
            instruction = f'Identity could not be verified from the camera. Give a neutral, warm Bengali greeting beginning with "{time_greeting}" in 1 short sentence. Do not call the person Pavel, Boss, or any name.'
        parts.append({'text': instruction})
        try:
            await self.session.send_client_content(turns={'role': 'user', 'parts': parts}, turn_complete=True)
            pass
        except Exception as _ge: print(f'[StartupFaceGreeting Error]: {_ge}')
    ...
    async def _run_system_monitor(self) -> None:
        """৫ মিনিট পর পর চেক করবে এবং শুধুমাত্র অতিরিক্ত লোড থাকলে সংক্ষেপে সতর্ক করবে।"""
        while True:
            ...
            with self._speaking_lock:
                speaking = self._is_speaking
            if speaking or (time.monotonic() - self._last_user_speech) < 20:
                continue
            alert = await asyncio.to_thread(self._sys_monitor.check)
            if alert:
                try:
                    msg = f"System warning: {alert}. Inform Boss in 1 short, polite Bengali sentence."
                    await self.session.send_client_content(turns={"role": "user", "parts": [{"text": msg}]}, turn_complete=True)
                except Exception as e:
                    print(f"[SystemMonitor Alert]: {e}")
    ...
    async def _run_proactive_mode(self) -> None:
        """৫ মিনিট পর পর ক্যামেরা ও স্ক্রিন দেখে ইউজারের সাথে নিজে থেকে কথা বলবে।"""
        while True:
            await asyncio.sleep(300)
            if not self.session or not self._awake:
                continue
            with self._speaking_lock:
                speaking = self._is_speaking
            if speaking or (time.monotonic() - self._last_user_speech) < 180:
                continue
            try:
                import base64 as _b64, cv2, gc, numpy as np
                ...
                def _get_cam():
                    ...
                    cap.release()
                    if not ret or frame is None: return None
                    _, enc = cv2.imencode('.jpg', frame, [int(cv2.IMWRITE_JPEG_QUALITY), 65])
                    return enc.tobytes()
                def _get_scr():
                    try:
                        scr = ImageGrab.grab().resize((800, 450))
                        frame = cv2.cvtColor(np.array(scr), cv2.COLOR_RGB2BGR)
                        _, enc = cv2.imencode('.jpg', frame, [int(cv2.IMWRITE_JPEG_QUALITY), 60])
                        return enc.tobytes()
                    except Exception: return None
                c_bytes = await asyncio.to_thread(_get_cam)
                s_bytes = await asyncio.to_thread(_get_scr)
                parts = []
                if c_bytes:
                    parts.append({"inline_data": {"mime_type": "image/jpeg", "data": _b64.b64encode(c_bytes).decode('ascii')}})
                if s_bytes:
                    parts.append({"inline_data": {"mime_type": "image/jpeg", "data": _b64.b64encode(s_bytes).decode('ascii')}})
                gc.collect()
                if not parts: continue
                try:
                    from core.face_id import check_if_pavel
                    verified = await asyncio.wait_for(
                        asyncio.to_thread(check_if_pavel, c_bytes),
                        timeout=2.0,
                    ) == "pavel"
                except Exception:
                    verified = False
                if not verified:
                    print("[Proactive] Identity not verified; no Boss-specific greeting.")
                    continue
                prompt = "The webcam identity was verified as the primary user. Look at the webcam image and screen image. See what the primary user is doing or working on. Initiate a sweet, caring, and short conversation in pure Bengali. Address only the verified primary user as Boss. 1 or 2 natural sentences only."
                parts.append({"text": prompt})
                await self.session.send_client_content(turns={"role": "user", "parts": parts}, turn_complete=True)
                print("[Proactive] Checked screen and camera, spoken to Boss.")
            except Exception as e:
                ...
                async with (
                   ...
                   # tg.create_task(self._run_sleep_watch())
                    if self._dashboard:
                      ...
                      # নেক্সা অন হতেই ফেস চেক এবং কুশল বিনিময় চালু
                    tg.create_task(self._startup_face_check())
