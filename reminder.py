...
def _write_notify_script(task_name: str, message: str, os_name: str, base_dir: Path) -> Path:
...
# 2. Native Windows Speech Synthesis (Speaks out loud reliably)
try:
    safe_msg = msg.replace('"', '\\"').replace("'", "\\'")
    ps_cmd = f"Add-Type -AssemblyName System.Speech; $synth = New-Object System.Speech.Synthesis.SpeechSynthesizer; $synth.Speak('{{safe_msg}}')"
    subprocess.run(["powershell", "-NoProfile", "-NonInteractive", "-Command", ps_cmd], creationflags=0x08000000)
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    if result.returncode != 0:
        ...
        print(f"[Reminder] ❌ schtasks: {result.stderr.strip()}")
        return ""  
...
def reminder(
    ...
    delay_raw = parameters.get("seconds_from_now", "")

    if str(delay_raw).strip() not in ("", "0"):
        try:
            delay_seconds = float(delay_raw)
        except (TypeError, ValueError):
            return "I couldn't understand that relative delay."
        if delay_seconds <= 0:
            return "I need a positive amount of time for a relative reminder."
    ...
    try:
        script_path = _write_notify_script(task_name, safe_msg, os_name, _base_dir())
    except Exception as e:
...
TOOL = {
    "name": "reminder",
    "description": (
        "Sets a timed reminder using Task Scheduler. For a RELATIVE reminder "
        "('in 10 seconds', 'in 5 minutes', 'in 2 hours'), pass ONLY "
        "seconds_from_now and message."
    ),
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "date": {
                "type": "STRING",
                "description": "Absolute date in YYYY-MM-DD format. Omit for relative reminders."
            },
            "time": {
                "type": "STRING",
                "description": "Absolute time in HH:MM format (24h). Omit for relative reminders."
            },
            "seconds_from_now": {
                "type": "NUMBER",
                "description": "For relative reminders only: seconds from now to fire."
            },
            "message": {
                "type": "STRING",
